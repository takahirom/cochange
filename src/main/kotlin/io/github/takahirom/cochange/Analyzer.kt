...
object CouplingKind {
    ...
    fun of(a: String, b: String, category: String, namesRelated: Boolean): Estimate {
        ...
        return when {
            ...
            // A lockfile is not a definition that declares anything: it is a resolved
            // snapshot. `package-lock.json × App.kt` was described as "a build definition
            // and the code it declares", which is false of the lockfile.
            (FileRole.of(a) == FileRole.LOCKFILE || FileRole.of(b) == FileRole.LOCKFILE) -> Estimate(
                "lockfile", "none",
                "one side is a dependency lockfile — a resolved snapshot regenerated by the package manager, not something anyone edits by design. The coupling is the dependency update itself.",
            )
